...
internal class FaceAnalyser {
    ...
    fun analyzeFaces(
        ...
        val facePortraits: List<FacePortrait> = if (faceOptions.prominentFaceDetection) {
            analyseProminentFace(faces, bitmap, faceOptions,context)
        } else {
            analyseMultipleFaces(faces, bitmap, faceOptions,context)
        }
    ...
    private fun processFace(
        ...
        if (faceSize >= minFaceSize) {
            ...
            portraitData?.let {
                ...
                val genderRange = getGender(croppedBitmap,context )
    ...
    private fun getGender(bitmap: Bitmap, context: Context): String {
        ViolaGenderClassifier().initialize(context)
        val result = ViolaGenderClassifier.classifier!!.recognizeImage(bitmap, 0)
        return result[0].range
    }
}